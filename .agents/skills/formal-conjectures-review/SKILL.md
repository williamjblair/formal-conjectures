...
lists the ones that recur, and
[`references/definition-traps.md`](references/definition-traps.md) records the ones this
repository has already been burned by; read it now.
...
End the report with reviewer attribution and the evidence needed to assess the
work: reviewer kind (`human` or `ai`), named attribution, method, exact Git
repository/commit/path/content-hash inputs, scope, independence or named shared
dependencies, and content-addressed result paths. Human and AI are peer
attributed kinds. Do not use the kind as a quality score; quality comes from
the method, exact inputs, scope, independence/dependencies, and results.

Keep the detailed record in the source repository or an existing Git/Entire
history. Do not create an attempt ID, session object, or separate memory store
for the review. Attribution does not change the verdict into maintainer
acceptance, and the skill still does not decide whether to merge.
