...
Run the `review` skill against the finished file as if it were someone else's
submission, and fix what it finds before opening the pull request. The review
procedure's step 4 checks are the complement of this skill's step 3 table, and
the pass is cheap next to a review round-trip with a maintainer.
