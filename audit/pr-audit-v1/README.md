...
Repository owner and name are retained input bindings, not schema constants. This permits a pinned contributor-fork pull request to use the same v1 core and observation contracts. The request identity, normalized repository result, PR URL, receipt variables, base/head commits, trees, changed paths, and blobs must all name the same repository and exact revision. An advisory source-fidelity method may likewise carry retained source references and an exact-head review result. Such a result remains AI-prepared, non-independent evidence and cannot produce a `clean` disposition or a maintainer decision.
