...
A missing required method/configuration file is a malformed input packet and generation refuses it; it is not silently converted to an `unavailable` check. A producer-level missing tool may be `unavailable` only when the packet retains a committed execution procedure and a real attempted invocation with the exact resolved command/path, environment, exit status, stdout, and stderr. The Rupert fixture does **not** claim such an invocation: it records only that an exact Comparator execution identity is absent from the retained packet. The missing-tool execution gate therefore remains explicitly unmet, separately from unavailable exact proof-artifact identity.
...
| `unavailable-rupert-3959` | exact-head build passes, but the proof metadata names only a mutable repository root rather than an exact commit/file | `unavailable` |
