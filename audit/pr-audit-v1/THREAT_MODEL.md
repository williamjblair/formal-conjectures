...
9. A producer relabels a corrupt packet or unattempted command as a missing tool -> `unavailable` hides a producer defect or proof failure -> consumers stop distinguishing absence, error, and failed comparison.
...
| TM-011 | Buggy or malicious producer | Can author normalized tool-result input | Label missing input, a skipped command, or a failed proof as `unavailable` | Audit hides producer failure and misstates scientific evidence | Tool provenance, review trust | Comparator availability profile requires one exact retained procedure, a real inert invocation, explicit resolution/process/exit/stdout/stderr fields, and fixed nonclaims; generator and standalone core validation reject outcome/profile drift | A rooted invocation record is not a hardware attestation and still depends on producer custody | Run capture in a protected no-secret job; attest producer identity; keep command fixed and inert; retain raw host/process evidence outside the core if later execution needs stronger provenance | Mutate invocation-attempted, process-start, error class, roots, and typed result in hostile tests; alert on unavailable rates by producer/environment | medium | medium | medium |
