...
def _validate_legacy_source_statement_fidelity_profile(
...
def _validate_advisory_source_statement_fidelity_profile(
    check: Mapping[str, Any],
    repository: Mapping[str, Any],
    descriptors_by_id: Mapping[str, Mapping[str, Any]],
    loaded: Mapping[str, tuple[Mapping[str, Any], Any, bytes]] | None = None,
) -> None:
    """Validate a retained, exact-head advisory source review packet."""

    implementation = check["implementation"]
    if (
        check["id"] != "source-statement-fidelity"
        or check["kind"] != "semantic"
        or check["mode"] != "human_review"
        or check["role"] != "advisory"
        or check["scope"]["revision"] != "head"
        or len(check["scope"]["paths"]) != 1
        or len(check["scope"]["declarations"]) != 1
        or implementation["name"] != "retained-advisory-source-fidelity-review"
        or implementation["version"] != "1"
        or implementation["kind"] != "human_review_guide"
        or implementation["locator"] != "source-fidelity-method.json"
    ):
        raise AuditError("advisory source-statement fidelity check has an unsupported profile")

    path = check["scope"]["paths"][0]
    declaration = check["scope"]["declarations"][0]
    change = next((item for item in repository["changes"] if item["path"] == path), None)
    if change is None or change["head_blob_sha256"] is None:
        raise AuditError("advisory source-statement fidelity scope is not an exact head source")

    relations = {item["id"]: item for item in check["inputs"]}
    head = relations.get("head-source")
    method = relations.get("source-fidelity-method")
    review = relations.get("source-fidelity-review")
    source_relations = sorted(
        (item for item in check["inputs"] if item["kind"] == "source-reference"),
        key=lambda item: _utf16_sort_key(item["id"]),
    )
    if (
        head is None
        or head["kind"] != "git-blob"
        or head["locator"] != f"{path}@{repository['head']['commit_oid']}"
        or head["root"] != change["head_blob_sha256"]
        or method is None
        or method["kind"] != "human-review-guide"
        or method["locator"] != implementation["locator"]
        or method["root"] != implementation["root"]
        or descriptors_by_id[method["artifact_id"]]["role"] != "method"
        or review is None
        or review["kind"] != "advisory-review-observation"
        or descriptors_by_id[review["artifact_id"]]["role"] != "tool_output"
        or not source_relations
        or any(descriptors_by_id[item["artifact_id"]]["role"] != "source_file" for item in source_relations)
    ):
        raise AuditError("advisory source-statement fidelity inputs are incomplete or drifted")
    if len(check["evidence"]) != 1 or (
        check["evidence"][0]["locator"], check["evidence"][0]["sha256"]
    ) != (review["locator"], review["root"]):
        raise AuditError("advisory source-statement fidelity evidence is not the retained review")

    if loaded is None:
        return

    method_value = _expect_dict(loaded[method["artifact_id"]][1], "source fidelity method")
    _expect_keys(
        method_value,
        required=(
            "schema_version", "name", "version", "operation", "authority", "repository",
            "pull_request", "scope", "source_inputs", "required_edge_cases", "nonclaims",
        ),
        location="source fidelity method",
    )
    expected_source_inputs = [
        {"id": item["id"], "locator": item["locator"], "root": item["root"]}
    ...
    expected_method = {
        ...
        "pull_request": {
            ...
            "base_commit_oid": repository["base"]["commit_oid"],
            "head_commit_oid": repository["head"]["commit_oid"],
        },
        "scope": {"path": path, "declaration": declaration},
        "source_inputs": expected_source_inputs,
    ...
    if method_value != expected_method:
        raise AuditError("source fidelity method does not match its exact repository, head, declaration, and source inputs")

    review_value = _expect_dict(loaded[review["artifact_id"]][1], "source fidelity review")
    _expect_keys(
        review_value,
        required=(
            "schema_version", "method_root", "preparer", "authority", "independent",
            "head_commit_oid", "head_blob_oid", "source_root", "scope", "outcome",
            "severity", "finding", "witness", "nonclaims",
        ),
        location="source fidelity review",
    )
    evidence = check["evidence"][0]
    expected_review = {
        "schema_version": "formal-conjectures.pr-audit-advisory-source-fidelity-review.v1",
        "method_root": implementation["root"],
        "preparer": "codex_ai_packet_preparer",
        "authority": "advisory_packet_preparation_only",
        "independent": False,
        "head_commit_oid": repository["head"]["commit_oid"],
        "head_blob_oid": change["head_blob_oid"],
        "source_root": change["head_blob_sha256"],
        "scope": check["scope"],
    ...
    if review_value != expected_review:
        raise AuditError("source fidelity review does not match the exact-head advisory check")
...
def _validate_source_statement_fidelity_profile(
    ...
    """Dispatch between the retained legacy human chain and advisory input profile."""

    if check["implementation"]["locator"] == "human-source-fidelity-method.json":
        _validate_legacy_source_statement_fidelity_profile(
            check, repository, descriptors_by_id, loaded
        )
        return
    if check["implementation"]["locator"] == "source-fidelity-method.json":
        _validate_advisory_source_statement_fidelity_profile(
            check, repository, descriptors_by_id, loaded
        )
        return
    raise AuditError("source-statement fidelity check has an unsupported retained method")
...
def _validate_implementation_binding(
    ...
    elif locator == "source-fidelity-method.json":
        expected = ("retained-advisory-source-fidelity-review", "1", "human_review_guide")
...
def validate_observation(value: Any) -> dict[str, Any]:
    ...
    url_for_receipt = _expect_https(pr_for_receipt.get("url"), "observation pull request URL")
    url_match = re.fullmatch(r"https://github\.com/([^/]+)/([^/]+)/pull/([1-9][0-9]*)", url_for_receipt)
    if url_match is None or int(url_match.group(3)) != number_for_receipt:
        raise AuditError("observation pull request URL cannot bind receipt repository variables")
    receipt = _validate_observation_receipt(
        source["receipt"],
        repository={
            "repository": {"owner": url_match.group(1), "name": url_match.group(2)},
            "pull_request": {"number": number_for_receipt},
        },
