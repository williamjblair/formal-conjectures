...
EXTERNAL_FORK = "external-fork-erdos-430-7"
...
class ExternalForkFixtureTest(unittest.TestCase):

    def test_external_fork_core_and_observation_bind_exact_inputs(self):
        directory = FIXTURES / EXTERNAL_FORK
        core = generate_core(directory / "core-input.json")
        self.assertEqual(framed(core), (directory / "expected-core.json").read_bytes())
        self.assertEqual(core["repository"]["repository"], {
            "host": "github.com", "owner": "Paul-Lez", "name": "formal-conjectures",
            "url": "https://github.com/Paul-Lez/formal-conjectures",
        })
        self.assertEqual(core["repository"]["pull_request"]["number"], 7)
        self.assertEqual(core["repository"]["base"]["commit_oid"], "398958d3964d738886bd24433918c365df4a2aab")
        self.assertEqual(core["repository"]["head"]["commit_oid"], "075c42c999c0c19224fd116d272637d7868df42a")
        source_check = next(check for check in core["checks"] if check["property"] == "source-statement-fidelity")
        self.assertEqual(source_check["scope"], {
            "revision": "head",
            "paths": ["FormalConjectures/ErdosProblems/430.lean"],
            "declarations": ["Erdos430.erdos_430"],
        })
        self.assertEqual(
            {item["id"] for item in source_check["inputs"] if item["kind"] == "source-reference"},
            {"source-current", "source-history", "source-original"},
        )
        observation = generate_observation(
            directory / "observation-input.json", directory / "expected-core.json"
        )
        self.assertEqual(framed(observation), (directory / "expected-observation.json").read_bytes())
        self.assertEqual(observation["source"]["receipt"]["variables"], {
            "owner": "Paul-Lez", "name": "formal-conjectures", "number": 7,
        })

    def test_external_fork_advisory_result_has_no_authority_effect(self):
        core = generate_core(FIXTURES / EXTERNAL_FORK / "core-input.json")
        source_check = next(check for check in core["checks"] if check["property"] == "source-statement-fidelity")
        typed_input = next(item for item in source_check["inputs"] if item["kind"] == "typed-result")
        typed = load(FIXTURES / EXTERNAL_FORK / "inputs" / f"typed-result-{source_check['id']}.json")
        self.assertEqual(typed_input["locator"], "typed-result/ai_review_preparer/codex_ai_packet_preparer")
        self.assertEqual(typed["semantic_review"]["authority"], "advisory_packet_preparation_only")
        self.assertFalse(typed["semantic_review"]["independent"])
        self.assertIsNone(typed["semantic_review"]["reviewer"])
        self.assertEqual(source_check["outcome"], "fail")
        self.assertEqual(source_check["severity"], "meaning")
        self.assertEqual(core["disposition"]["advisory"], "needs_revision")
        self.assertIn("not_an_acceptance_or_merge_decision", core["disposition"]["nonclaims"])
        self.assertNotIn("maintainer_disposition", core)

    def test_external_fork_stale_head_rebinding_is_refused(self):
        with tempfile.TemporaryDirectory() as temporary:
            directory = Path(temporary) / "fixture"
            shutil.copytree(FIXTURES / EXTERNAL_FORK, directory)
            repository_path = directory / "inputs" / "repository.json"
            repository = load(repository_path)
            repository["head"]["commit_oid"] = "0" * 40
            write_canonical(repository_path, repository)
            update_manifest_artifact(directory, "core-input.json", "inputs/repository.json")
            with self.assertRaisesRegex(
                AuditError,
                "(?:request identity variables|pull request identity|head commit|head-source relation)",
            ):
                generate_core(directory / "core-input.json")

    def test_external_fork_source_method_drift_is_refused(self):
        with tempfile.TemporaryDirectory() as temporary:
            directory = Path(temporary) / "fixture"
            shutil.copytree(FIXTURES / EXTERNAL_FORK, directory)
            method_path = directory / "inputs" / "source-fidelity-method.json"
            method = load(method_path)
            method["pull_request"]["head_commit_oid"] = "0" * 40
            write_canonical(method_path, method)
            update_manifest_artifact(directory, "core-input.json", "inputs/source-fidelity-method.json")
            with self.assertRaisesRegex(AuditError, "input .* root does not match retained artifact"):
                generate_core(directory / "core-input.json")
